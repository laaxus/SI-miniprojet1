...
void init_state() {
	my_sem_init_tts(&sem_read, 1);
	my_sem_init_tts(&sem_write, 1);
	my_mutex_init_tts(&mutex_readcount);
	my_mutex_init_tts(&mutex_writecount);
	my_mutex_init_tts(&mutex_z);
}
...
void* writer_main() {
	for(int i = 0; i < NBWRITING;i++)
	{
		...
		if (writecount == 1) {
			my_sem_wait_tts(&sem_read);
		}

		my_mutex_unlock_tts(&mutex_writecount);
		my_sem_wait_tts(&sem_write);
		...
		my_sem_post_tts(&sem_write);
		
		my_mutex_lock_tts(&mutex_writecount);
		
		writecount--;
		if (writecount == 0) {
			my_sem_post_tts(&sem_read);
		}
...
void* reader_main() {
	for(int i = 0; i < NBREADING;i++)
	{
		...
		my_mutex_unlock_tts(&mutex_readcount);
		my_sem_post_tts(&sem_read);
		my_mutex_unlock_tts(&mutex_z);
